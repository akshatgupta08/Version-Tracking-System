...
export default function Table({ data }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const wordsArray = data.split(' '); /*Get the Lender name and the version of the lender selected by the user.*/
     ...
     /*Data regarding a lender is received in the info object, which is then displayed by the table.*/
...
/*Each time new rows are created for a new lender and the rows are made editable*/
  useEffect(() => {
    if (tableRef.current) {
      editTable(tableRef.current);
    }
  }, [rows]);
/*The editTable function attaches an input box to the row which has been clicked.
When the update button is clicked, the new data goes to the backend to be stored.*/
